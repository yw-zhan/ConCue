...
class VCOCO(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, llm_object, llm_position, llm_posture, llm_spatial, transforms, num_queries, args):
        self.img_set = img_set
        self.img_folder = img_folder
        with open(anno_file, 'r') as f:
            self.annotations = json.load(f)
        with open(llm_object, 'r') as f:
            self.object_annotations = json.load(f)
        with open(llm_position, 'r') as f:
            self.position_annotations = json.load(f)
        with open(llm_posture, 'r') as f:
            self.posture_annotations = json.load(f)
        with open(llm_spatial, 'r') as f:
            self.spatial_annotations = json.load(f)
    ...
    def __getitem__(self, idx):
        ...
        llm_object = self.object_annotations[idx]
        llm_position = self.position_annotations[idx]
        llm_posture = self.posture_annotations[idx]
        llm_spatial = self.spatial_annotations[idx]
        ...
        return img, target, llm_object, llm_position, llm_posture, llm_spatial
...
def build(image_set, args):
    ...
    PATHS = {
        'train': (root / 'images' / 'train2014', root / 'annotations' / 'trainval_vcoco.json',
                  root / 'annotations/vlm/object_train_coco.json',
                  root / 'annotations/vlm/location_train_coco.json',
                  root / 'annotations/vlm/posture_train_coco.json',
                  root / 'annotations/vlm/spatial_train_coco.json'),
        'val': (
            root / 'images' / 'val2014', root / 'annotations' / 'test_vcoco.json',
            root / 'annotations/vlm/object_test_coco.json',
            root / 'annotations/vlm/location_test_coco.json',
            root / 'annotations/vlm/posture_test_coco.json',
            root / 'annotations/vlm/spatial_test_coco.json'),
    }
    CORRECT_MAT_PATH = root / 'annotations' / 'corre_vcoco.npy'

    img_folder, anno_file, llm_object, llm_position, llm_posture, llm_spatial = PATHS[image_set]
    dataset = VCOCO(image_set, img_folder, anno_file, llm_object, llm_position, llm_posture, llm_spatial, transforms=make_vcoco_transforms(image_set),
