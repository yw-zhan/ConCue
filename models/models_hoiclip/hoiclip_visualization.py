...
from .gen_visual import build_gen
...
class HOICLIP(nn.Module):
    ...
    def forward(self, samples: NestedTensor, processor, text_model, llm_object, llm_position, llm_posture, llm_spatial, is_training=True, clip_input=None, targets=None):
        ...
        objects = [i['output'] for i in llm_object]
        object_token = processor.batch_encode_plus(objects, padding='longest', return_tensors='pt').to(mask.device)
        object_mask = 1.0 - object_token['attention_mask']
        object_emb = text_model(**object_token)
        object_emb = object_emb.last_hidden_state
        position = [i['output'] for i in llm_position]
        position_token = processor.batch_encode_plus(position, padding='longest', return_tensors='pt').to(
            mask.device)
        position_mask = 1.0 - position_token['attention_mask']
        position_emb = text_model(**position_token)
        position_emb = position_emb.last_hidden_state
        posture = [i['output'] for i in llm_posture]
        posture_token = processor.batch_encode_plus(posture, padding='longest', return_tensors='pt').to(
            mask.device)
        posture_mask = 1.0 - posture_token['attention_mask']
        posture_emb = text_model(**posture_token)
        posture_emb = posture_emb.last_hidden_state
        spatial = [i['output'] for i in llm_spatial]
        spatial_token = processor.batch_encode_plus(spatial, padding='longest', return_tensors='pt').to(mask.device)
        spatial_mask = 1.0 - spatial_token['attention_mask']
        spatial_emb = text_model(**spatial_token)
        spatial_emb = spatial_emb.last_hidden_state


        h_hs, o_hs, inter_hs, clip_cls_feature, clip_hoi_score, clip_visual, weights_1, weight2_1, weight3_1, \
            weights_2, weight2_2, weight3_2, weights_3, weight2_3, weight3_3, weights_4, weight2_4, weight3_4 = self.transformer(self.input_proj(src), mask,
                                                self.query_embed_h.weight,
                                                self.query_embed_o.weight,
                                                self.pos_guided_embedd.weight,
                                                pos[-1], self.clip_model, self.clip_visual_proj, clip_input, object_emb,
                                                object_mask, position_emb, position_mask, posture_emb, posture_mask,
        ...
        path_to_datasets = '/data3/zhanyuwei/HOICLIP-main/data/hico_20160224_det/images/test2015/{}'
        # TODO: path to store visualization results
        root_cache = './vis/GEN_unseen_object_attn'

        llm_object_dir = '/data3/zhanyuwei/HOICLIP-main/data/hico_20160224_det/annotations/object_test.json'
        llm_position_dir = '/data3/zhanyuwei/HOICLIP-main/data/hico_20160224_det/annotations/location_test.json'
        llm_posture_dir = '/data3/zhanyuwei/HOICLIP-main/data/hico_20160224_det/annotations/postures_test.json'
        llm_spatial_dir = '/data3/zhanyuwei/HOICLIP-main/data/hico_20160224_det/annotations/spatial_test.json'

        with open(llm_object_dir, 'r') as f:
            object_annotations = json.load(f)
        with open(llm_position_dir, 'r') as f:
            position_annotations = json.load(f)
        with open(llm_posture_dir, 'r') as f:
            posture_annotations = json.load(f)
        with open(llm_spatial_dir, 'r') as f:
            spatial_annotations = json.load(f)

        for hoi, boxes_h, boxes_o, tgt, attn_w1, attn_wc1, attn_wp1, attn_w2, attn_wc2, attn_wp2, attn_w3, attn_wc3, attn_wp3, \
                attn_w4, attn_wc4, attn_wp4 in zip(out['pred_hoi_logits'], out['pred_sub_boxes'],
                                                               out['pred_obj_boxes'], targets, weights_1, weight2_1, weight3_1, \
                                                                weights_2, weight2_2, weight3_2, weights_3, weight2_3, weight3_3, \
                                                                weights_4, weight2_4, weight3_4):
            ...
            if score_hoi < 0.5:
                continue
            h_b = boxes_h[select_query]
            h_o = boxes_o[select_query]
            ...
            b1 = np.asarray(h_b.cpu())
            b2 = np.asarray(h_o.cpu())
            ...
            # self.visulize_attention_ratio(image_, attn_w1[select_query], os.path.join(
            #     cache_dir, "{}_{}_attn1.png".format(tgt['filename'], score_hoi)
            # ))

            mask_obj = attn_w1[select_query].cpu().float().numpy()
            normed_mask_obj = (mask_obj - mask_obj.min()) / (mask_obj.max() - mask_obj.min())
            # normed_mask_obj = mask_obj / mask_obj.max()
            mask_posture = attn_w2[select_query].cpu().float().numpy()
            # normed_mask_posture = mask_posture / mask_posture.max()
            normed_mask_posture = (mask_posture - mask_posture.min()) / (mask_posture.max() - mask_posture.min())
            mask_position = attn_w3[select_query].cpu().float().numpy()
            # normed_mask_position = mask_position / mask_position.max()
            normed_mask_position = (mask_position - mask_position.min()) / (mask_position.max() - mask_position.min())
            mask_spatial = attn_w4[select_query].cpu().float().numpy()
            # normed_mask_position = mask_position / mask_position.max()
            normed_mask_spatial = (mask_spatial - mask_spatial.min()) / (mask_spatial.max() - mask_spatial.min())
            data_dict = {}
            data_dict['filename'] = tgt['filename']
            for i, value in enumerate(object_annotations):
                if value['filename'] == data_dict['filename']:
                    data_dict['obj'] = object_annotations[i]['output']
                    data_dict['position'] = position_annotations[i]['output']
                    data_dict['posture'] = posture_annotations[i]['output']
                    data_dict['spatial'] = spatial_annotations[i]['output']
                    data_dict['mask_obj'] = normed_mask_obj.tolist()
                    data_dict['mask_position'] = normed_mask_position.tolist()
                    data_dict['mask_posture'] = normed_mask_posture.tolist()
                    data_dict['mask_spatial'] = normed_mask_spatial.tolist()
            ...
            # for visualize attention map
            cache_dir_clip = cache_dir.replace("_object_attn","_object_attn_clip")
            if not os.path.exists(cache_dir_clip):
                os.makedirs(cache_dir_clip)
            self.visulize_attention_ratio(image_, attn_wc1[select_query], os.path.join(
                cache_dir, "{}_{}_attn1.png".format(tgt['filename'], score_hoi)
            ))
            # print(attn_wc1[select_query].shape)
            # print(attn_wp1[select_query].shape)
            self.visulize_attention_ratio(image_, attn_wp1[select_query], os.path.join(
                cache_dir_clip, "{}_{}_attn1_clip.png".format(tgt['filename'], 1.0)
            ))
            image_.save(os.path.join(
                cache_dir, "{}_{}.png".format(tgt['filename'], score_hoi)))

            # for visualize attention map
            # self.visulize_attention_ratio(image_, attn_w2[select_query], os.path.join(
            #     cache_dir, "{}_{}_attn2.png".format(tgt['filename'], score_hoi)
            # ))

            # for visualize attention map
            self.visulize_attention_ratio(image_, attn_wc2[select_query], os.path.join(
                cache_dir, "{}_{}_attn2.png".format(tgt['filename'], score_hoi)
            ))
            self.visulize_attention_ratio(image_, attn_wp2[select_query], os.path.join(
                cache_dir_clip, "{}_{}_attn2_clip.png".format(tgt['filename'], 1.0)
            ))
            # for visualize attention map
            # self.visulize_attention_ratio(image_, attn_w3[select_query], os.path.join(
            #     cache_dir, "{}_{}_attn3.png".format(tgt['filename'], score_hoi)
            # ))

            # for visualize attention map
            self.visulize_attention_ratio(image_, attn_wc3[select_query], os.path.join(
                cache_dir, "{}_{}_attn3.png".format(tgt['filename'], score_hoi)
            ))
            self.visulize_attention_ratio(image_, attn_wp3[select_query], os.path.join(
                cache_dir_clip, "{}_{}_attn3_clip.png".format(tgt['filename'], 1.0)
            ))

            self.visulize_attention_ratio(image_, attn_wc4[select_query], os.path.join(
                cache_dir, "{}_{}_attn4.png".format(tgt['filename'], score_hoi)
            ))
            self.visulize_attention_ratio(image_, attn_wp4[select_query], os.path.join(
                cache_dir_clip, "{}_{}_attn4_clip.png".format(tgt['filename'], 1.0)
            ))
    ...
    def visulize_attention_ratio(self, img, attention_mask, path, ratio=1.0, cmap="jet"):
        ...
        # print(mask.size)
        normed_mask = (mask - mask.min()) / (mask.max() - mask.min())
        # normed_mask = np.array(F.softmax(torch.tensor(mask)))
...
def build(args):
    ...
    text_model = RobertaModel.from_pretrained("/data3/zhanyuwei/HOICLIP-main/data/roberta-base/").cuda()

    for p in text_model.parameters():
        p.requires_grad_(False)

    processor = RobertaTokenizer.from_pretrained("/data3/zhanyuwei/HOICLIP-main/data/roberta-base/")

    return model, criterion, postprocessors, processor, text_model
