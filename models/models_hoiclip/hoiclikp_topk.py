...
class HOICLIP(nn.Module):
    def __init__(self, backbone, transformer, num_queries, aux_loss=False, args=None):
        ...
        if args.with_clip_label:
            ...
            self.hoi_class_embedding = nn.Linear(args.clip_embed_dim, 600)
        ...
        #     for hoi_pair in hoi_text_label_del:
    ...
    def forward(self, samples: NestedTensor, processor, text_model, llm_object, llm_position, llm_posture, llm_spatial,
                is_training=True, clip_input=None, targets=None):
        ...
        h_hs, o_hs, inter_hs, clip_cls_feature, clip_hoi_score, clip_visual = self.transformer(self.input_proj(src),
                                                                                               mask,
                                                                                               self.query_embed_h.weight,
                                                                                               self.query_embed_o.weight,
                                                                                               self.pos_guided_embedd.weight,
                                                                                               pos[-1], self.clip_model,
                                                                                               self.clip_visual_proj,
                                                                                               clip_input, object_emb,
                                                                                               object_mask,
                                                                                               position_emb,
                                                                                               position_mask,
                                                                                               posture_emb,
        ...
        out_hoi_logits = out['pred_hoi_logits']
        out_obj_logits = out['pred_obj_logits']
        ...
        for index in range(len(hoi_scores)):
            hoi_scores_index = hoi_scores[index].to('cpu')
            obj_scores_index = obj_scores[index].to('cpu')
            obj_scores_index = obj_scores_index * obj_scores_index
            hoi_scores_index = hoi_scores_index + obj_scores_index[:, self.hoi_obj_list]
            hoi_labels_index = np.tile(np.arange(hoi_scores_index.shape[1]), (hoi_scores_index.shape[0], 1))
            hoi_scores_index = hoi_scores_index.ravel()
            hoi_labels_index = hoi_labels_index.ravel()

            max_hois = 3
            topk_hoi_scores = top_k(list(hoi_scores_index), max_hois)
            # print(topk_hoi_scores)
            topk_indexes = np.array([np.where(hoi_scores_index == score)[0][0] for score in topk_hoi_scores])

            hoi_labels_topk = hoi_labels_index[topk_indexes]
            # print(hoi_labels_topk)
            # hoi_class = hico_text_label.values()
            hoi_class = [hico_text_label[id] for id in hico_text_label.keys()]
            # print(hoi_class)
            data_dict = {}
            data_dict['filename'] = targets[index]['filename']
            hoi_class_index = [hoi_class[i] for i in hoi_labels_topk]
            data_dict['output'] = hoi_class_index
            data_anno.append(data_dict)
            # for i in hoi_labels_topk:
            #     print(hoi_class[int(i)])
        seen_inter_dumps = json.dumps(data_anno)
        a = open('HCVC_default.json', 'w')
