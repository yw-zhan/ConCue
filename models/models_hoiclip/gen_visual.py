...
class GEN(nn.Module):
    ...
    def forward(self, src, mask, query_embed_h, query_embed_o, pos_guided_embed, pos_embed, clip_model, clip_proj,
        ...
        ins_hs4 = self.instance_decoder4(ins_tgt, memory, sup_memory=spatial_states_sup, sup_memory_mask=spatial_mask,
                                       memory_key_padding_mask=mask,
                                       pos=pos_embed, query_pos=ins_query_embed)
        # ins_hs = torch.cat((ins_hs1, ins_hs2, ins_hs3), dim=-1)
        # print(inter_hs1.shape)
        # print(ins_hs1.shape)
        # ins_hs = self.ins_class_ln(self.ins_class_fc(ins_hs))
        ...
        inter_hs1, weights_1, weight2_1, weights3_1 = self.clip_interaction_decoder1(inter_hs.permute(1, 0, 2), clip_visual.permute(1, 0, 2),
                                                  sup_memory=object_states, sup_memory_mask=object_mask, sup_memory2=memory)
        inter_hs2, weights_2, weight2_2, weights3_2 = self.clip_interaction_decoder2(inter_hs.permute(1, 0, 2), clip_visual.permute(1, 0, 2),
                                                  sup_memory=posture_states, sup_memory_mask=posture_mask, sup_memory2=memory)
        inter_hs3, weights_3, weight2_3, weights3_3 = self.clip_interaction_decoder3(inter_hs.permute(1, 0, 2), clip_visual.permute(1, 0, 2),
                                                  sup_memory=position_states, sup_memory_mask=position_mask, sup_memory2=memory)
        inter_hs4, weights_4, weight2_4, weights3_4 = self.clip_interaction_decoder4(inter_hs.permute(1, 0, 2), clip_visual.permute(1, 0, 2),
        ...
        return h_hs, o_hs, inter_hs, clip_cls_feature, clip_hoi_score, clip_visual @ clip_proj.to(dtype),\
            weights_1, weight2_1[:, :, 1:].unflatten(2, (7, 7)), weights3_1.unflatten(2, (h, w)), \
            weights_2, weight2_2[:, :, 1:].unflatten(2, (7, 7)), weights3_2.unflatten(2, (h, w)), \
            weights_3, weight2_3[:, :, 1:].unflatten(2, (7, 7)), weights3_3.unflatten(2, (h, w)),\
            weights_4, weight2_4[:, :, 1:].unflatten(2, (7, 7)), weights3_4.unflatten(2, (h, w))
    ...
    def forward(self, tgt, memory, sup_memory=None, sup_memory_mask=None, sup_memory2=None,
        ...
        for i, layer in enumerate(self.layers):
            if len(output.shape) == 4:
                ...
                output = output
            output, weight, weight2, weight3 = layer(output, memory, sup_memory=sup_memory, sup_memory_mask=sup_memory_mask, sup_memory2=sup_memory2, tgt_mask=tgt_mask,
        ...
        if self.return_intermediate:
            return torch.stack(intermediate), weight, weight2, weight3

        return output, weight, weight2, weight3
...
class TransformerDecoderFusionLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory, sup_memory=None, sup_memory_mask=None, sup_memory2=None,
        ...
        if self.enable_cp:
            ...
            tgt2, weight2 = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos),
        ...
        tgt3, weight3 = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos),
        ...
        tgt3, weight = self.multihead_attn2(query=self.with_pos_embed(tgt2, query_pos),
                                   key=self.with_pos_embed(sup_memory, pos),
                                   value=sup_memory, attn_mask=memory_mask,
                                   key_padding_mask=sup_memory_mask)
        ...
        tgt = self.norm3(tgt)
        return tgt, weight, weight2, weight3
