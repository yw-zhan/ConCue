...
from datasets.datasets_gen.hico_eval_ko import HICOEvaluator as HICOEvaluator_gen
...
def train_one_epoch(model: torch.nn.Module, processor, text_model, criterion: torch.nn.Module,
    ...
    debug_count = 0
    step = 0
    for samples, targets, llm_object, llm_position, llm_posture, llm_spatial in metric_logger.log_every(data_loader, print_freq, header):
        if no_training:
        ...
        outputs = model(samples, processor, text_model, llm_object, llm_position, llm_posture, llm_spatial, clip_input=clip_img, targets=targets)
...
@torch.no_grad()
def evaluate_hoi(dataset_file, processor, text_model, model, postprocessors, data_loader,
    ...
    for samples, targets, llm_object, llm_position, llm_posture, llm_spatial in metric_logger.log_every(data_loader, 10, header):
        samples = samples.to(device)
        clip_img = torch.stack([v['clip_inputs'] for v in targets]).to(device)

        outputs = model(samples, processor, text_model, llm_object, llm_position, llm_posture, llm_spatial, is_training=False, clip_input=clip_img, targets=targets)
    ...
    if args.training_free_enhancement_path:
        ...
        text_hoi_feature = model.module.transformer.hoi_cls.T.cpu()
