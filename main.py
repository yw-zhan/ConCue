...
def get_args_parser():
    ...
    parser.add_argument('--lr_drop_gamma', default=0.2, type=float)
...
def main(args):
    ...
    model, criterion, postprocessors, processor, text_model = build_model(args)
    model.to(device)
    print('****************')
    # print(model)
    ...
    if args.eval:
        ...
        if 'Test result:' not in previous_log:
            ...
            test_stats = evaluate_hoi(args.dataset_file, processor, text_model, model, postprocessors, data_loader_test,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, processor, text_model, criterion, data_loader_train, optimizer, device, epoch, args.clip_max_norm,
            lr_scheduler,
        ...
        if epoch <= 50 and epoch % 5 == 0:
            test_stats = evaluate_hoi(args.dataset_file, processor, text_model, model, postprocessors, data_loader_val,
                                      args.subject_category_id, device, args)
            if args.output_dir and utils.is_main_process():
                #  add eval in log for my convenience
                with (output_dir / "log.txt").open("a") as f:
