...
from engine_cues import train_one_epoch, evaluate_hoi
...
def get_args_parser():
    ...
    parser.add_argument('--visual_cues', default='object', type=str,
                        help="object, posture, location, blip")
    ...
    parser.add_argument('--num_prediction', type=int)
...
def main(args):
    ...
    model, criterion, postprocessors, tokenizer, llama_model = build_model(args)
    model.to(device)
    print('****************')
    # print(model)
    ...
    # train dataloader initializationc
    dataset_train = build_dataset(image_set='train', args=args)

    if args.distributed:
        sampler_train = DistributedSampler(dataset_train, shuffle=False, drop_last=False)
    ...
    if args.eval:
        ...
        if 'Test result:' not in previous_log:
            ...
            test_stats = evaluate_hoi(args.dataset_file, model, tokenizer, llama_model, postprocessors, data_loader_test,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(model, tokenizer, llama_model, data_loader_train, device, epoch, args)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('HCVC training and evaluation script', parents=[get_args_parser()])
