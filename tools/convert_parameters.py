...
def main(args):
    ...
    for k in list(ps['model'].keys()):
        ...
        if len(k.split('.')) > 1 and k.split('.')[1] == 'decoder':
            ps['model'][k.replace('decoder', 'instance_decoder1')] = ps['model'][k].clone()
            ps['model'][k.replace('decoder', 'instance_decoder2')] = ps['model'][k].clone()
            ps['model'][k.replace('decoder', 'instance_decoder3')] = ps['model'][k].clone()
            ps['model'][k.replace('decoder', 'instance_decoder4')] = ps['model'][k].clone()
            del ps['model'][k]
